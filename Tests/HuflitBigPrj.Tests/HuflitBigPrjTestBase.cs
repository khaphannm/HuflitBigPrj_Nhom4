...
{
    public abstract class HuflitBigPrjTestBase : AbpIntegratedTestBase<HuflitBigPrjTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected HuflitBigPrjTestBase()
        {
        ...
        protected void UsingDbContext(Action<HuflitBigPrjDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<HuflitBigPrjDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HuflitBigPrjDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HuflitBigPrjDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HuflitBigPrjDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HuflitBigPrjDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<HuflitBigPrjDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HuflitBigPrjDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HuflitBigPrjDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HuflitBigPrjDbContext, Task<T>> func)
        {
            T result;
